...
def test_get_default_template(monkeypatch):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data.get("template_key") == "obscura_01"
...
def test_get_template(monkeypatch):
    ...
    response = client.get("/api/templates/obscura_01")
    assert response.status_code == 200
    data = response.json()
    assert data.get("template_key") == "obscura_01"
