...
This guide will walk you through setting up and installing Obscura.

## Prerequisites

Before you begin, ensure you have the following prerequisites installed and configured:

1.  **Podman or Docker:**  Obscura uses containerization for easy deployment. Install either [Podman](https://podman.io/) or [Docker](https://www.docker.com/). I like Podman because it is rootless and daemonless.

2.  **LLM Endpoint:** Obscura supports multiple types of LLM providers. The model you choose must support tool calling:
...
3.  **Whisper-compatible Transcription Service:** Obscura needs a service to transcribe audio into text. I recommend self-hosted solutions:
...
4.  **Hardware Considerations:**
    ...
    - **Quantization:** Using Q4 quantization can significantly reduce memory usage and improve token generation speed without significant degrading output quality.
          ...
          - **KV Cache Quantization:** This is an advanced configuration option that can further reduce memory usage. Aggressive KV cache quantization (smaller than Q8) is not recommended for heavily context-dependent tasks like those in Obscura.
...
5.  **Access Obscura in your Browser:**
    Open your web browser and navigate to [http://localhost:3000](http://localhost:5000).

6.  **Initial Configuration:**
    Once Obscura is running, access the **Settings** page within the application:
...
### Configuration Tips
- Enable diarization in Obscura settings for speaker-aware transcripts
...
⚠️ **By default, Obscura binds to 0.0.0.0 (all network interfaces) for development convenience.**

**If exposed to the internet without protection:**
   - Anyone can access your instance
   - All data could be stolen

**Never expose Obscura to the open internet without some form of reverse proxy (Nginx/Caddy) or VPN.**
...
If you encounter problems, review the local server and browser logs first.
